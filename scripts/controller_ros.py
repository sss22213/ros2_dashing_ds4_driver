...
import rclpy
from sensor_msgs.msg import BatteryState, Joy, JoyFeedback, JoyFeedbackArray, Imu
...
class ControllerRos(Controller):
    def __init__(self,ros2_node):
        ...
        self._prev_joy = None
        '''
        self.use_standard_msgs = False
        self.deadzone = 0.1
        self.frame_id = 'ds4'
        self.imu_frame_id = 'ds4_imu'
        self.pub_joy_on_change = True
        self._prev_joy = None
        ##
        # self.Laser_pub = self.create_subscription(Image,"laser_image",self.Laser_Stream_subscriber_callback)
        # self.image_pub = self.create_publisher(Image,"debug_circle_image_tracker")

        # Use ROS-standard messages (like sensor_msgs/Joy)
        if self.use_standard_msgs:
            self.pub_report = ros2_node.create_publisher(Report,'raw_report')
            self.pub_battery = ros2_node.create_publisher(BatteryState,'battery')
            self.pub_joy = ros2_node.create_publisher(Joy,'joy')
            self.pub_imu = ros2_node.create_publisher(Imu,'imu')
            self.sub_feedback = ros2_node.create_subscription(JoyFeedbackArray,'set_feedback', self.cb_joy_feedback)
            '''
            ...
            self.pub_status = ros2_node.create_publisher(Status,'status')
            self.sub_feedback = ros2_node.create_subscription(Feedback,'set_feedback', self.cb_feedback)
            '''
